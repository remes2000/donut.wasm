...
void rotate(float* rotation_accumulator, float xRotateAngle, float yRotateAngle, float* result) {
  float partial[3 * 3];
  float rotateXAxis[] = {
    1.0f, 0.0f, 0.0f,
  ...
  multiply(rotation_accumulator, rotateXAxis, partial, 3, 3, 3, 3);
...
char* render_frame(float* rotation_accumulator, float screen_width, float screen_height, float r1, float r2, float distance) {
  ...
  for (float theta=0; theta < 2 * M_PI; theta += theta_spacing) {
    for (float phi=0; phi < 2 * M_PI; phi += phi_spacing) {
      ...
      float position[3];
      multiply(donutRotate, rotation_accumulator, partial, 3, 3, 3, 3);
